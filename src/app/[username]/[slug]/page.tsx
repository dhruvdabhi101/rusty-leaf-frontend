...
async function getData(username: string, slug: string) {
    ...
    const author_data = await fetch(`http://localhost:8000/user/${post.user_id.$oid}`);
...
export default async function Page({ params }: { params: { username: string, slug: string } }) {
    ...
    return (
        <div className="flex flex-col p-3 md:justify-center md:items-center">
            <div className="">
                <p className="text-3xl font-bold text-[#ffc799]">{post.title}</p>
            ...
            '>
                <p dangerouslySetInnerHTML={{ __html: post.content }} className="">
                </p>
            </div>
